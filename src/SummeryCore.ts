...
import { getConfigFile } from './lib/configLoader';
import { IMiddleware } from 'koa-router';
import { isEmptyObject, loadAllScriptFileOnDir } from './util/util';
import { resolve } from 'path';
import { routerMap, Controller, Get, Post, Auth, RequestMapping, Required } from './lib/decorators';
import { RouterMapping } from './lib/route';
...
export class Summery{
  ...
  constructor(configFileName:string="project.config.json"){
    //加载配置文件
    this.config=getConfigFile(configFileName);
